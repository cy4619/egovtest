...
public class SecuredObjectDAO  implements ApplicationContextAware {
	 ...
	 * @throws Exception
	 */
	public LinkedHashMap<Object, List<ConfigAttribute>> getRolesAndResources(
			String resourceType, String requestMatcherType) throws Exception {
		...
		List<Map<String, Object>> resultList = this.namedParameterJdbcTemplate
				.queryForList(sqlRolesAndResources,
		...
		while (itr.hasNext()) {
			...
			if (isResourcesUrl) {
				if (requestMatcherType.equalsIgnoreCase("regex")) {
					presentResource = new SelfRegexRequestMatcher(
							presentResourceStr, null);
				} else if (requestMatcherType.equalsIgnoreCase("ciRegex")) {
					presentResource = new SelfRegexRequestMatcher(
							presentResourceStr, null, true);
				} else {
					presentResource = new AntPathRequestMatcher(
							presentResourceStr);
				}
			} else {
			...
			// LinkedList<ConfigAttribute>();
			//
			// // 이미 requestMap 에 해당 Resource 에 대한 Role 이 하나 이상 맵핑되어 있었던 경우,
			// // sort_order 는 resource(Resource) 에 대해 매겨지므로 같은 Resource 에 대한
			// Role 맵핑은 연속으로 조회됨.
			// // 해당 맵핑 Role List (SecurityConfig) 의 데이터를 재활용하여 새롭게 데이터 구축
			// if (preResource != null &&
			// presentResourceStr.equals(preResource)) {
			...
			// }
			//
			// configList.add(new SecurityConfig((String)
			// tempMap.get("authority")));
			//
...
/*    public LinkedHashMap<Object, List<ConfigAttribute>> getRolesAndResources(String resourceType, String requestMatcherType) throws Exception {
        ...
        for (Map.Entry<Object, List<ConfigAttribute>> entry: resourcesMap.entrySet()) {
